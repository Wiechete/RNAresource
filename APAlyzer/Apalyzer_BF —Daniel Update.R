...
if(batch_mode_on){
	...
	controlCount <- as.numeric(parsed_args[1])
	treatmentCount <- as.numeric(parsed_args[2])
...
# technically should be set via the parameter passed Rsubread::featureCounts
numCores <- detectCores()
numProcesses <- length(names(flsall))
if((numCores - 1) < numProcesses)
{
	numThreads <- numCores - 1
}else
{
	numThreads <- numProcesses
}
...
# extracting the number of treatment and control groups from sample_names_metadata file
# by checking whether sample name contain string "control"

# controlCount <- sum(grepl("Control", new_sample_names))
# treatmentCount <- length(new_sample_names) - controlCount

treatmentGroup <- "Treatment"
controlGroup <- "Control"

sampleTable = data.frame(samplename = c(names(flsall)),
